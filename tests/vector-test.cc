...
void TestVector() {
  ...
  ASSERT(vec.io_size() == sizeof(uint64_t));
  ...
  ASSERT(vec.io_size() == sizeof(uint64_t) + ((sizeof(int) * values.size())));
...
void TestFlatVector() {
  ...
  ASSERT(vec.empty());
  ASSERT(vec.total_size() == 0);
  ASSERT(vec.io_size() == (sizeof(uint64_t) * 3));

  marisa::grimoire::Vector<uint32_t> values;
  vec.build(values);
  ...
  ASSERT(vec.io_size() == (sizeof(uint64_t) * 4));
  ASSERT(vec[0] == 0);
  ...
  values.clear();
  for (std::size_t i = 0; i < 10000; ++i) {
    values.push_back(static_cast<uint32_t>(random_engine()));
  }
  vec.build(values);
...
void TestBitVector(std::size_t size) {
  ...
  ASSERT(bv.io_size() == sizeof(uint64_t) * 5);
