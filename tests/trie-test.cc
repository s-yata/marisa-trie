...
void TestTextTail() {
  ...
  marisa::grimoire::Vector<uint32_t> offsets;
  ...
  ASSERT(tail.io_size() == (sizeof(uint64_t) * 6));
  ...
  ASSERT(!tail.empty());
  ASSERT(tail.total_size() == tail.size());
  ASSERT(tail.io_size() == (sizeof(uint64_t) * 7));
...
void TestBinaryTail() {
  ...
  ASSERT(tail.size() == 1);
  ASSERT(!tail.empty());
  ASSERT(tail.total_size() == (tail.size() + sizeof(uint64_t)));
  ASSERT(tail.io_size() == (sizeof(uint64_t) * 8));
