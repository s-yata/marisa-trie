...
std::size_t select_bit(std::size_t i, std::size_t bit_id, uint64_t unit) {
  ...
  uint8_t skip;
  {
    ...
    skip = static_cast<uint8_t>(
        popcount(static_cast<uint64_t>(_mm_cvtsi128_si64(x))));
  }
...
std::size_t select_bit(std::size_t i, std::size_t bit_id, uint32_t unit_lo,
  ...
  {
   #ifdef _MSC_VER
    __declspec(align(16)) uint8_t unit_bytes[16];
    __declspec(align(16)) uint8_t accumulated_counts_bytes[16];
   #else   // _MSC_VER
    uint8_t unit_bytes[16] __attribute__((aligned(16)));
    uint8_t accumulated_counts_bytes[16] __attribute__((aligned(16)));
   #endif  // _MSC_VER
